...
plot4 <-function(raw_data){ 
  ...
  if(!is.null(raw_data)){
    ...
    #1
    with (graph_data4, plot(datetime,as.numeric(Global_active_power)/1000, type="n",xlab="", ylab="Global Active Power(killowatts)" ))
    ...
    legend("topright",col=c("black","red","blue"), pch="_", bty = "n",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))
    #3
    with(graph_data4, plot(datetime,Voltage,type="n"))
    with(graph_data4, lines(datetime,Voltage))
    #4
    with(graph_data4, plot(datetime,Global_reactive_power,type="n"))
